...
<head>
    <meta charset="UTF-8">
    <title>OCABIS <?= ($isViewer || $isAdmin || $isSuperAdmin) ? 'Item List' : 'Department' ?></title>
    ...
    <style>
        ...
        /* Hide department search field for viewers - they can still search items */
        body[data-user-is-viewer="true"] #treeSearch,
        body[data-user-is-viewer="true"] .sidebar-tree .search-container { display: none !important; }
        ...
        @media (max-width: 768px) {
            ...
            /* No filter message mobile */
            #noFilterMessage {
                padding: 30px 15px !important;
                margin-top: 20px !important;
            }

            #noFilterMessage > div[style*="font-size: 48px"] {
            ...
            .filters-section > div[style*="position: relative"] {
                width: 100% !important;
            }

            .filters-section > div[style*="position: relative"] input {
                width: 100% !important;
            ...
            /* Item card meta row mobile */
            .item-card-meta {
                flex-direction: column !important;
                gap: 6px !important;
            }

            /* Action buttons in item cards mobile */
            .item-card-actions {
                flex-direction: column !important;
                gap: 8px !important;
            }
    ...
    <!-- Define toggleShowAllItems function in separate script to ensure it loads first -->
    <script>
    // Define toggleShowAllItems function immediately - must be in global scope
    function toggleShowAllItems() {
        // Wait for DOM to be ready and variables to be defined
        if (typeof showAllItems === 'undefined') {
            // If showAllItems is not defined yet, wait a bit and try again
            setTimeout(toggleShowAllItems, 100);
            return;
        }
        
        // Toggle the global showAllItems variable
        showAllItems = !showAllItems;
        console.log('toggleShowAllItems - showAllItems toggled to:', showAllItems);
        
        // Update button appearance
        const btn = document.getElementById('showAllItemsBtn');
        const icon = document.getElementById('showAllItemsIcon');
        const text = document.getElementById('showAllItemsText');
        
        if (btn && icon && text) {
            if (showAllItems) {
                // Active state - show all items
                btn.style.background = '#10b981';
                btn.style.color = 'white';
                text.textContent = 'SHOWING ALL ITEMS';
                ...
                // Inactive state - filter by department
                btn.style.background = '#3b82f6';
                btn.style.color = 'white';
                text.textContent = 'SHOW ALL ITEMS';
                // Change icon back to list/grid icon
        ...
        const userIsDeptHead = (document.body?.dataset?.userIsDepartmentHead || 'false') === 'true';
        
        // Force refresh by resetting the flag first and clearing container
        if (typeof isUpdatingCardView !== 'undefined') {
        ...
        // - When showAllItems is FALSE (blue button): Hide everything, show empty state
        // - When showAllItems is TRUE (green button): Show items from their department
        if (userIsDeptHead) {
            if (!showAllItems) {
                // Blue button: Hide cards and table, show empty state
                // Clear current table selection and update button text back to "ADD ITEM TABLE"
                ...
                // Show the static noFilterMessage
                const noFilterMessage = document.getElementById('noFilterMessage');
                if (noFilterMessage) {
                    noFilterMessage.style.display = 'block';
                }
            } else {
                // Green button: Show items from their department
                if (cardsContainer && typeof updateCardView === 'function') {
                    cardsContainer.style.display = 'grid';
                    cardsContainer.style.visibility = 'visible';
                    // Hide the static noFilterMessage
                    const noFilterMessage = document.getElementById('noFilterMessage');
                    if (noFilterMessage) {
                        noFilterMessage.style.display = 'none';
                    }
                    // Small delay to ensure flag is reset, then update
                    setTimeout(() => {
                        updateCardView();
                    }, 100);
                }
                // If table view is visible, refresh it
                if (tableContainer && tableContainer.style.display !== 'none') {
                    // Ensure items are loaded before populating table
                    if (typeof loadAllItems === 'function' && (!allItems || allItems.length === 0)) {
                        loadAllItems().then(() => {
                            if (typeof populateTableFromCards === 'function') {
                                populateTableFromCards();
                            }
                        });
                    } else if (typeof populateTableFromCards === 'function') {
                        populateTableFromCards();
                    }
                }
            }
        } else {
            // For super admins and other non-department heads
            if (showAllItems) {
                // Green button: Show all items (no department filter)
                if (cardsContainer && typeof updateCardView === 'function') {
                    cardsContainer.style.display = 'grid';
                    cardsContainer.style.visibility = 'visible';
                    // Hide the static noFilterMessage
                ...
                // Also refresh items table view if it's visible
                if (tableContainer && tableContainer.style.display !== 'none') {
                    // Ensure items are loaded before populating table
                    if (typeof loadAllItems === 'function' && (!allItems || allItems.length === 0)) {
                        loadAllItems().then(() => {
                ...
                // Blue button: Only show if there's a search term
                if (cardsContainer && typeof updateCardView === 'function') {
                    cardsContainer.style.display = 'grid';
                    cardsContainer.style.visibility = 'visible';
                    setTimeout(() => {
                ...
                // Blue button with no search: Hide items, show empty state
                if (cardsContainer) {
                    cardsContainer.style.display = 'none';
                    cardsContainer.style.visibility = 'hidden';
                }
        ...
        // If there's a search term, also refresh the search results
        if (searchTerm && typeof performSearch === 'function') {
            performSearch();
        }
        
        // Update summary after a short delay to ensure data is loaded
        setTimeout(() => {
  ...
  <div class="logo-text">
    ...
    <li class="nav-item">
        <a href="department.php" class="nav-link active">
            <span class="nav-icon">
                <img src="image/department.png" alt="<?= ($isViewer || $isAdmin || $isSuperAdmin) ? 'Item List' : 'Department' ?>">
            </span>
            <?= ($isViewer || $isAdmin || $isSuperAdmin) ? 'Item List' : 'Department' ?>
        </a>
    </li>
    <?php if ($isDepartmentHead): ?>
    <li class="nav-item">
        <a href="head_borrow_items.php" class="nav-link">
            <span class="nav-icon">
...
<div class="sign-out">
            ...
            <div class="content-area">
                ...
                <div class="sidebar-tree">
                    ...
                    <div class="tree-menu" id="treeMenu">
                        <!-- All Departments/Item Tables Option - Only show for super admins -->
                        <?php if ($isSuperAdmin): ?>
                        <div class="tree-item" data-dept-id="all">
                            <div class="tree-node active" onclick="selectDepartment('all', '<?= $isViewer ? 'All Item Tables' : 'All Departments' ?>')">
                                <img src="image/building-1062.png" alt="Building" class="tree-icon">
                                <span class="tree-text"><?= $isViewer ? 'All Item Tables' : 'All Departments' ?></span>
                            </div>
                        </div>
                        <?php endif; ?>
                ...
                <div class="data-section">
                    <div class="filters-section">
                        ...
                        <!-- Show All Items Toggle Button -->
                        <?php 
                        // Show for department heads, super admins, and admins
                        $showAllItemsButton = $isDepartmentHead || $isSuperAdmin || $isAdmin;
                        ?>
                        <?php if ($showAllItemsButton): ?>
                        <button class="btn btn-show-all" id="showAllItemsBtn" onclick="toggleShowAllItems()" style="display: inline-flex; align-items: center; gap: 6px; background: #3b82f6; color: white; padding: 8px 16px; border: none; border-radius: 6px; font-size: 13px; font-weight: 600; cursor: pointer; transition: all 0.2s; margin-right: 10px;" title="Show All Items">
                            <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor" id="showAllItemsIcon">
                                <path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm0 18c-4.41 0-8-3.59-8-8s3.59-8 8-8 8 3.59 8 8-3.59 8-8 8zm-1-13h2v6h-2zm0 8h2v2h-2z"/>
                            </svg>
                            <span id="showAllItemsText">SHOW ALL ITEMS</span>
                        </button>
                        <?php endif; ?>
    ...
    <tr>
        <?php if (!$isViewer): ?>
        <th class="checkbox-column" onclick="event.stopPropagation();">
            <input type="checkbox" id="selectAllItems" onclick="event.stopPropagation();" onchange="toggleSelectAllItems()" style="cursor: pointer; pointer-events: auto; position: relative; z-index: 12;" />
        </th>
        <?php endif; ?>
        ...
        <th onclick="setSort('item_code')" class="sortable">
            Item Code <span class="sort-indicator" data-col="item_code"></span>
        </th>
        ...
        <th>Action</th>
                            ...
                            <tbody id="itemsTableBody">
                                <tr>
                                    <td colspan="<?php echo $isViewer ? '10' : '11'; ?>" class="no-items" style="text-align: center; padding: 20px;">
                                        <?= $isDepartmentHead ? 'Loading items...' : ($isViewer ? 'Select an item table to view items or click "All Item Tables"' : 'Select a department to view items or click "All Departments"') ?>
                                    </td>
                                </tr>
                            </tbody>
                        </table>
        ...
        <div class="modal">
            ...
            <div class="modal-body">
                ...
                <form id="addItemTableForm">
                    <div id="addItemTableError" style="display:none;margin-bottom:10px;padding:8px 10px;border-radius:6px;background:#fee2e2;color:#991b1b;font-size:13px;"></div>
                    <div class="form-group">
...
<div class="modal-overlay" id="archiveItemModal">
    <div class="modal">
        <div class="modal-header" style="background: linear-gradient(135deg, #e53e3e, #c53030); color: white;">
            <h3 style="color: white;" id="archiveItemTitle">Archive Item</h3>
        ...
        <div class="modal-body">
            <div style="text-align: center; padding: 20px 0;">
                ...
                <h4 style="margin: 0 0 12px 0; color: #333; font-size: 20px; font-weight: 600;">Are you sure you want to archive?</h4>
...
const IS_VIEWER = (document.body?.dataset?.userIsViewer || 'false') === 'true';
let selectedDepartmentId = 'all';
let selectedDepartmentName = IS_VIEWER ? 'All Item Tables' : 'All Departments';
let selectedCategory = null;
let allItems = [];
let showAllItems = false; // Toggle to show all items regardless of department filter
...
// Note: toggleShowAllItems function is defined at the beginning of the script tag

// Load items when page loads
document.addEventListener('DOMContentLoaded', function() {
        // Ensure page title is "Items" for department heads
        if (window.isDepartmentHead) {
    ...
    function buildTreeStructure() {
        ...
        // Only show "All Departments/Item Tables" for super admins
        if (isSuperAdmin) {
            // Check if "all" is the selected department
            const isAllActive = currentSelectedDeptId === 'all';
            const allDeptText = IS_VIEWER ? 'All Item Tables' : 'All Departments';
            treeHTML = `
            <!-- All Departments/Item Tables Option -->
            <div class="tree-item" data-dept-id="all">
                <div class="tree-node ${isAllActive ? 'active' : ''}" onclick="selectDepartment('all', '${allDeptText}')">
                    <img src="image/building-1062.png" alt="Building" class="tree-icon">
                    <span class="tree-text">${allDeptText}</span>
                </div>
            </div>
        `;
        }
...
function validateRequiredFields(formId) {
    ...
    requiredFields.forEach(field => {
        // Skip validation for hidden fields (like status field for consumable items)
...
function setButtonLoading(buttonId, isLoading, originalText = null) {
    ...
    // Check if showAllItems is enabled
    const showAllItemsEnabled = typeof showAllItems !== 'undefined' && showAllItems === true;
    const userIsDeptHead = (document.body?.dataset?.userIsDepartmentHead || 'false') === 'true';
    ...
    // If showAllItems is enabled (and not department head), show items from selected department immediately
    if (showAllItemsEnabled && !userIsDeptHead) {
    ...
    // Update breadcrumb - HIDE ENTIRE BREADCRUMB
    function updateBreadcrumb(deptName, categoryName = null, tableName = null) {
        ...
        // Hide the entire breadcrumb section
        if (breadcrumb) {
            breadcrumb.style.display = 'none';
        }
        
        // Also hide all individual elements
        if (separator) separator.style.display = 'none';
        if (deptBreadcrumb) deptBreadcrumb.style.display = 'none';
        if (tableSeparator) tableSeparator.style.display = 'none';
        if (tableBreadcrumb) tableBreadcrumb.style.display = 'none';
    }
    ...
    function goToItemsPage() {
        ...
        // Update button text back to "ADD ITEM TABLE" when going back to default view
        updateAddItemButtonText(false);
        ...
        if (selectedDepartmentId && selectedDepartmentId !== 'all') {
            ...
            const allDeptText = IS_VIEWER ? 'All Item Tables' : 'All Departments';
            updateBreadcrumb(allDeptText, null, null);
        }
    ...
    // If showAllItems is enabled, show all items regardless of department
    if (showAllItems) {
...
function toggleActionMenu(itemId) {
    ...
    function showNoItems(message) {
        ...
        const colspan = isViewer ? 10 : 11;
        document.getElementById('itemsTableBody').innerHTML = `
            <tr><td colspan="${colspan}" class="no-items" style="text-align: center; padding: 20px;">${message}</td></tr>
        `;
    }
    ...
    if (tableContainer && getComputedStyle(tableContainer).display !== 'none') {
        ...
        let items = allItems || [];
        // Only filter by department if showAllItems is false
        if (!showAllItems && selectedDepartmentId !== 'all') {
        ...
        // Count item tables filtered by department
        ...
        try {
            ...
            if (data.success && data.item_tables) {
                // Filter item tables by department
                const userIsDeptHead = (document.body?.dataset?.userIsDepartmentHead || 'false') === 'true';
                let filteredTables = data.item_tables;
                
                if (userIsDeptHead && typeof USER_DEPARTMENT_ID !== 'undefined' && USER_DEPARTMENT_ID !== null) {
                    // For department heads: count only their department's item tables
                    filteredTables = filteredTables.filter(table => {
                        const tableDeptId = String(table.department_id || '');
                        const userDeptId = String(USER_DEPARTMENT_ID);
                        return tableDeptId === userDeptId;
                    });
                } else if (selectedDepartmentId !== 'all' && selectedDepartmentId) {
                    // For others: count item tables from selected department
                    filteredTables = filteredTables.filter(table => {
                        const tableDeptId = String(table.department_id || '');
                        const selectedDeptId = String(selectedDepartmentId);
                        return tableDeptId === selectedDeptId;
                    });
                }
                
                itemTableCount = filteredTables.length;
                
                // Count total items across all item tables in the department
                let totalItemsCount = 0;
                let totalItemsQuantity = 0;
                
                // Count items from each filtered table
                const itemCountPromises = filteredTables.map(async (table) => {
                    try {
                        ...
                        if (itemsData.success && itemsData.items) {
                            return {
                                ...
                                quantity: itemsData.items.reduce((sum, item) => sum + (parseInt(item.quantity) || 0), 0)
                            };
                        }
                    } catch (error) {
                        console.error(`Error getting items for table ${table.id}:`, error);
                    }
                    return { count: 0, quantity: 0 };
                });
                
                const itemCounts = await Promise.all(itemCountPromises);
                itemCounts.forEach(result => {
                    totalItemsCount += result.count;
                    totalItemsQuantity += result.quantity;
                });
            ...
            // Fallback: count unique item tables from items data (already filtered by department)
            ...
            // Fallback: use items from allItems array
...
function updateSortIndicators() {
    ...
    function formatDate(dateString) {
        ...
        if (typeof dateString === 'string' && dateString.match(/^\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}$/)) {
            // MySQL DATETIME format - replace space with T to create ISO format
        ...
        // Format with explicit year to ensure correct year display
    ...
    function addNewItemTable() {
        ...
        const inlineError = document.getElementById('addItemTableError');
        if (inlineError) {
        ...
        if (!validateRequiredFields('addItemTableForm')) {
            if (inlineError) {
                inlineError.textContent = 'Please fill in all required fields before adding the item table.';
            ...
            if (!name || !category || !department) {
                if (inlineError) {
                    inlineError.textContent = 'Please fill in Item Table Name, Category, and Department.';
                    inlineError.style.display = 'block';
                }
                return;
            }
        ...
        .then(data => {
            ...
            if (data.success) {
                ...
                if (inlineError) {
                    inlineError.textContent = data.message || 'An error occurred while adding the item table.';
                    inlineError.style.display = 'block';
                    // Scroll to top of modal so error is visible
                    inlineError.scrollIntoView({ behavior: 'smooth', block: 'start' });
                }
            }
        })
        .catch(error => {
            ...
            console.error('Error:', error);
            if (inlineError) {
                inlineError.textContent = `An error occurred while adding the item table. ${error.message || ''}`;
                inlineError.style.display = 'block';
                inlineError.scrollIntoView({ behavior: 'smooth', block: 'start' });
            }
        });
            ...
            .then(data => {
                ...
                if (data.success && data.item_tables) {
                    ...
                    tables.forEach(table => {
                        ...
                        option.setAttribute('data-is-consumable', table.is_consumable || '0');
    ...
    function onItemTableChange() {
        ...
        if (selectedOption.value === '') {
            ...
            // Show status field when no table is selected
        ...
        // Handle status field visibility based on consumable status
        const statusGroup = document.getElementById('itemStatus').closest('.form-group');
        const statusSelect = document.getElementById('itemStatus');
        
        if (isConsumable) {
            // Hide status field for consumable tables
            if (statusGroup) {
                statusGroup.style.display = 'none';
            }
            // Set status to Consumable (hidden field will be used)
            if (statusSelect) {
                statusSelect.value = 'Consumable';
            }
        } else {
            // Show status field for non-consumable tables
            if (statusGroup) {
                statusGroup.style.display = '';
            }
            // Reset to default status if not set
    ...
    function closeEditItemModal() {
        ...
        const descriptionTextarea = document.getElementById('editItemDescription');
...
function addNewItem() {
    ...
    // Check if selected table is consumable
    const selectedOption = itemTable.options[itemTable.selectedIndex];
    const isConsumable = selectedOption && selectedOption.getAttribute('data-is-consumable') === '1';
    
    // For consumable items, status is automatically set to "Consumable" and field is hidden
    // So we don't need to validate status field for consumable items
    if (isConsumable && itemStatus) {
        // Ensure status is set to Consumable for consumable items
        itemStatus.value = 'Consumable';
    }
    
    // Validate required values (skip status validation for consumable items)
    const statusValid = isConsumable ? true : (itemStatus && itemStatus.value);
    if (!itemTable.value || !itemName.value.trim() || !itemDepartmentId.value || !itemCategory.value || !itemQuantity.value || !itemLocation.value || !statusValid) {
    ...
    // Validate item name format (selectedOption already declared above)
    const tableName = selectedOption.getAttribute('data-table-name');
    const baseItemName = tableName.replace(' Table', '').replace(' table', '');
    ...
    const locationValue = itemLocation.value.trim();
    console.log('Location field value:', locationValue);
    console.log('Location field selectedIndex:', itemLocation.selectedIndex);
    console.log('Location field options:', Array.from(itemLocation.options).map(opt => ({value: opt.value, text: opt.text})));
    
    if (!locationValue || locationValue === '') {
        modal.error('Please select a location for the item.');
        submitBtn.disabled = false;
        submitBtn.textContent = originalText;
        return;
    }
    ...
    .then(data => {
        ...
        if (data.success) {
            ...
            // If user is viewing a specific item table, immediately hide cards and show table
            if (window.currentTableId && window.currentTableName) {
                const cardsContainer = document.getElementById('itemsCardsContainer');
                const tableContainer = document.querySelector('.table-container');
                
                // Immediately hide cards and show table to prevent card view from appearing
                if (cardsContainer) {
                    cardsContainer.style.display = 'none';
                    cardsContainer.style.visibility = 'hidden';
                }
            ...
            // Reload items and refresh view
            loadAllItems().then(() => {
                // If user is viewing a specific item table, refresh that table view
                if (window.currentTableId && window.currentTableName) {
                    console.log('Refreshing table view after adding item:', window.currentTableId, window.currentTableName);
...
function downloadQrFromDetail_OLD() {
                ...
                fetch(`crud.php?action=check_item_table_consumable&item_table_id=${item.item_table_id}`)
                    ...
                    .then(data => {
                        if (data.success && data.is_consumable) {
                            // Item is consumable - disable ALL fields except description and image
                            // Status must remain "Consumable" and cannot be changed
                            statusSelect.value = 'Consumable';
                            statusSelect.disabled = true;
                            statusSelect.style.backgroundColor = '#f3f4f6';
                            statusSelect.style.cursor = 'not-allowed';
                            
                            // Disable name, department, category, and location
                            ...
                            // Description and image can still be edited
                            ...
                            consumableNote.innerHTML = '⚠️ This item belongs to a consumable table. Status must remain "Consumable". Name, department, category, and location cannot be changed. Only description and image can be edited.';
                        } else {
    ...
    function updateItem() {
        ...
        .then(data => {
            if (data.success) {
                ...
                // Reload all items and refresh current view
                loadAllItems().then(() => {
                    // Check if we're viewing a specific item table - refresh that view
                    if (window.currentTableId && window.currentTableName) {
                        if (typeof showTableForItemTable === 'function') {
                            showTableForItemTable(window.currentTableId, window.currentTableName);
                        }
                    } else {
                        // Refresh table view if it's visible
                        const tableContainer = document.querySelector('.table-container');
                        if (tableContainer && tableContainer.style.display !== 'none' && typeof populateTableFromCards === 'function') {
                            populateTableFromCards();
                        }
                        
                        // Refresh cards view if it's visible
                        const cardsContainer = document.getElementById('itemsCardsContainer');
                        if (cardsContainer && cardsContainer.style.display !== 'none' && typeof updateCardView === 'function') {
                            updateCardView();
                        }
                        
                        // If search is active, refresh search results
                        const searchInput = document.getElementById('nameFilter');
                        if (searchInput && searchInput.value.trim() && typeof performSearch === 'function') {
                            performSearch();
                        }
                    }
                });
    ...
    // Archive item - Store ID(s) for confirmation
    let itemToArchiveId = null;
    let itemsToArchiveArray = null; // For multiple items
    ...
    function archiveItem(id) {
        ...
        // Store the ID for later use (single item mode)
        itemToArchiveId = id;
        itemsToArchiveArray = null;
        ...
        document.getElementById('archiveItemTitle').textContent = 'Archive Item';
    ...
    function archiveSelectedItemsModal() {
        ...
        // Store the array for multiple items mode
        itemsToArchiveArray = Array.from(selectedItems);
        itemToArchiveId = null;
        
        // Update modal content for multiple items
        const count = itemsToArchiveArray.length;
        document.getElementById('archiveItemName').textContent = `${count} item(s) will be moved to the archive.`;
        document.getElementById('archiveItemTitle').textContent = `Archive ${count} Item(s)`;
    ...
    function closeArchiveModal() {
        ...
        // If single item was being archived, uncheck the checkbox
        if (itemToArchiveId) {
            const checkbox = document.querySelector(`input.item-checkbox[value="${itemToArchiveId}"]`);
            if (checkbox) {
    ...
    function confirmArchive() {
        ...
        if (itemsToArchiveArray && itemsToArchiveArray.length > 0) {
            let completed = 0;
            let failed = 0;
            
            const archiveNext = (index) => {
                if (index >= itemsToArchiveArray.length) {
                    // All items processed
                    ...
                    // Reload items without page reload
                    loadAllItems().then(() => {
                        // Check if we're viewing a specific item table - preserve that view
                        if (window.currentTableId && window.currentTableName) {
                            // Stay in the same table view
                            if (typeof showTableForItemTable === 'function') {
                                showTableForItemTable(window.currentTableId, window.currentTableName);
                            }
                        } else {
                    ...
                    if (failed === 0) {
                        modal.success(`${completed} item(s) have been archived successfully!`);
                    } else {
                        modal.warning(`${completed} item(s) archived successfully, ${failed} item(s) failed to archive.`);
                    }
                    return;
                }
                
                const itemId = itemsToArchiveArray[index];
        ...
        .then(data => {
            ...
            if (data.success) {
                // Remove from selectedItems if it was there
                if (itemToArchiveId && selectedItems.has(itemToArchiveId)) {
                    selectedItems.delete(itemToArchiveId);
                    updateSelectionUI();
                    updateSelectedCount();
                }
                ...
                // Immediately reload items (no delay, no page reload)
                loadAllItems().then(() => {
                    // Check if we're viewing a specific item table - preserve that view
                    if (window.currentTableId && window.currentTableName) {
                        // Stay in the same table view
...
function closeRequestModal() {
    ...
    async function showAndFilterItems(searchTerm) {
        ...
        // Always show item tables (not individual items) when searching
        if (tableContainer) {
            tableContainer.style.display = 'none';
        }
        ...
        try {
            ...
            console.log('showAndFilterItems - showAllItems:', showAllItems, 'userIsDeptHead:', userIsDeptHead, 'USER_DEPARTMENT_ID:', typeof USER_DEPARTMENT_ID !== 'undefined' ? USER_DEPARTMENT_ID : 'undefined');
            
            // When searching, always allow search results regardless of showAllItems state
            // But still apply department filtering based on user role and settings
            if (userIsDeptHead && typeof USER_DEPARTMENT_ID !== 'undefined' && USER_DEPARTMENT_ID !== null) {
                // For department heads: always filter by their department when searching
                const originalCount = tablesToShow.length;
                console.log('showAndFilterItems - Department head - Filtering by department - Total tables:', originalCount, 'USER_DEPARTMENT_ID:', USER_DEPARTMENT_ID);
                tablesToShow = tablesToShow.filter(table => {
                    const tableDeptId = String(table.department_id || '');
                    const userDeptId = String(USER_DEPARTMENT_ID);
                    const matches = tableDeptId === userDeptId;
                    if (!matches) {
                        console.log('showAndFilterItems - Filtered out table:', table.table_name, 'department_id:', table.department_id, 'expected:', USER_DEPARTMENT_ID);
                    }
                    return matches;
                });
                console.log(`showAndFilterItems - Department head filtering: ${originalCount} tables -> ${tablesToShow.length} tables`);
            } else if (!showAllItems && selectedDepartmentId !== 'all' && selectedDepartmentId) {
                // For non-department heads: filter by selected department if showAllItems is false
                const originalCount = tablesToShow.length;
                tablesToShow = tablesToShow.filter(table => {
                    const tableDeptId = String(table.department_id || '');
                    const selectedDeptId = String(selectedDepartmentId);
                ...
                console.log(`showAndFilterItems - Department filtering: ${originalCount} tables -> ${tablesToShow.length} tables`);
            } else {
                console.log('showAndFilterItems - Showing all item tables (no department filtering)');
            }
                ...
                try {
                    ...
                    if (itemsData.success && itemsData.items) {
                        hasMatchingItems = itemsData.items.some(item => {
                            ...
                            const itemDepartment = (item.department_name || '').toLowerCase();
                            const itemLocation = (item.location || '').toLowerCase();
                            const searchableText = `${itemName} ${itemCode} ${itemCategory} ${itemDepartment} ${itemLocation}`;
    ...
    async function displayFilteredItemTables(tables) {
        ...
        const cardsPromises = tables.map(async (table) => {
            ...
            // Check if user is a viewer/teacher
            const isViewer = document.body && document.body.dataset && document.body.dataset.userIsViewer === 'true';
            ...
            try {
                ...
                if (countData.success && countData.items) {
                    ...
                    // For viewers/teachers, count only items with status "Working"
                    if (isViewer) {
            ...
            return `
                <div class="item-card clickable-card" data-table-id="${table.id}" ${!isViewer ? `onclick="showTableForItemTable('${table.id}', '${table.table_name}')"` : ''} style="${isViewer ? 'cursor: default;' : ''}">
                    ...
                    <div class="item-card-content">
                        <div class="item-title-row">
                            ...
                            <div class="card-action-dropdown">
                                ...
                                <div class="card-action-menu" id="card-menu-${table.id}">
                                    <button onclick="${!isViewer ? `showTableForItemTable('${table.id}', '${table.table_name}')` : `requestBorrowFromTable('${table.id}', event)`}">
                                        ${isViewer ? `
                        ...
                        <div class="quantity-text">${isViewer ? `Available Items: ${availableItemCount}` : `Items: ${itemCount}`}</div>
...
function toggleItemSelection(itemId, event) {
    ...
    // Toggle selection - add or remove from selectedItems
...
function showTableView() {
    ...
    // Hide the static noFilterMessage immediately
    const noFilterMessage = document.getElementById('noFilterMessage');
    if (noFilterMessage) {
        noFilterMessage.style.display = 'none';
    }
    ...
    // Populate the table with current items immediately
    populateTableFromCards();
    
    // Don't call performSearch here - it might show the message again
    // The table is already populated by populateTableFromCards()
...
async function updateCardView() {
    // Prevent duplicate calls (but allow if showAllItems state changed)
    ...
    const userIsDeptHead = (document.body?.dataset?.userIsDepartmentHead || 'false') === 'true';
    console.log('updateCardView called - showAllItems:', showAllItems, 'isDepartmentHead:', userIsDeptHead, 'USER_DEPARTMENT_ID:', typeof USER_DEPARTMENT_ID !== 'undefined' ? USER_DEPARTMENT_ID : 'undefined');
    const cardsContainer = document.getElementById('itemsCardsContainer');
    ...
    try {
        ...
        // If no item tables in database, show empty state
        if (!data.success || !data.item_tables || data.item_tables.length === 0) {
            cardsContainer.innerHTML = `
                <div class="no-items-card">
                    <div class="no-items-icon">📦</div>
        ...
        // Debug: Log current state
        console.log('updateCardView - showAllItems:', showAllItems, 'userIsDeptHead:', userIsDeptHead, 'USER_DEPARTMENT_ID:', typeof USER_DEPARTMENT_ID !== 'undefined' ? USER_DEPARTMENT_ID : 'undefined');
        
        // For department heads: 
        // - When showAllItems is FALSE (blue button): Show NO items (empty state)
        // - When showAllItems is TRUE (green button): Show items from their department only
        // For others: filter by selected department if showAllItems is false
        if (userIsDeptHead && typeof USER_DEPARTMENT_ID !== 'undefined' && USER_DEPARTMENT_ID !== null) {
            if (!showAllItems) {
                // Blue button: Show NO items (empty state)
                console.log('Department head - Blue button: Showing NO items (empty state)');
                tablesToShow = [];
            } else {
                // Green button: Show items from their department only
                const originalCount = tablesToShow.length;
                console.log('Department head - Green button: Filtering by department - Total tables:', originalCount, 'USER_DEPARTMENT_ID:', USER_DEPARTMENT_ID);
                tablesToShow = tablesToShow.filter(table => {
                    const tableDeptId = String(table.department_id || '');
                    const userDeptId = String(USER_DEPARTMENT_ID);
                    const matches = tableDeptId === userDeptId;
                    if (!matches) {
                        console.log('Filtered out table:', table.table_name, 'department_id:', table.department_id, 'expected:', USER_DEPARTMENT_ID);
                    }
                    return matches;
                });
                console.log(`Department head filtering: ${originalCount} tables -> ${tablesToShow.length} tables (department_id: ${USER_DEPARTMENT_ID})`);
            }
        } else {
            // For super admins and other non-department heads
            // Even when showAllItems is true, still respect the selected department from tree
            // Only show all items if "All Departments" is selected
            if (selectedDepartmentId === 'all') {
                // "All Departments" selected - show all item tables
                console.log('Show All Items is enabled with "All Departments" - showing all item tables (no filtering)');
            } else if (selectedDepartmentId && selectedDepartmentId !== 'all') {
                // Specific department selected - filter by that department even if showAllItems is true
                const originalCount = tablesToShow.length;
                tablesToShow = tablesToShow.filter(table => {
                    const tableDeptId = String(table.department_id || '');
                    const selectedDeptId = String(selectedDepartmentId);
                ...
                console.log(`Show All Items enabled but filtering by selected department: ${originalCount} tables -> ${tablesToShow.length} tables (department_id: ${selectedDepartmentId})`);
            } else if (!showAllItems) {
                // showAllItems is false - filter by selected department if one is selected
                if (selectedDepartmentId !== 'all' && selectedDepartmentId) {
                    ...
                    console.log(`Department filtering: ${originalCount} tables -> ${tablesToShow.length} tables (department_id: ${selectedDepartmentId})`);
                }
            }
        }
        
        // If no tables to show after filtering, show empty state
        if (tablesToShow.length === 0) {
            ...
            // Show the static noFilterMessage instead of creating a duplicate
            const noFilterMessage = document.getElementById('noFilterMessage');
            if (noFilterMessage) {
                noFilterMessage.style.display = 'block';
            }
        ...
        // If we have items to show, hide the static noFilterMessage
        const noFilterMessage = document.getElementById('noFilterMessage');
        if (noFilterMessage) {
            noFilterMessage.style.display = 'none';
        }
        ...
        // Clear container first to prevent duplicates (double-check)
        if (cardsContainer) {
            cardsContainer.innerHTML = '';
        }
        ...
        // Double-check container is still empty before adding
        if (cardsContainer && cardsContainer.innerHTML.trim() !== '') {
            console.log('Warning: Container not empty, clearing again');
            cardsContainer.innerHTML = '';
        }
        ...
        console.log('updateCardView completed - showing', tablesToShow.length, 'item tables (showAllItems:', showAllItems, ')');
...
async function deleteItemTable(tableId, tableName) {
    try {
        let proceed = true;
        const message = `Delete item table "${tableName}"? This cannot be undone.`;
        if (typeof modal !== 'undefined' && typeof modal.confirm === 'function') {
            proceed = await modal.confirm(message, 'Delete Item Table');
        } else {
        ...
        const data = await resp.json();
        if (data.success) {
            if (typeof modal !== 'undefined' && typeof modal.success === 'function') {
                modal.success(data.message || 'Item table deleted');
            }

            // Reload items and refresh both cards and summary counts WITHOUT full page refresh
            ...
            if (typeof modal !== 'undefined' && typeof modal.error === 'function') {
    ...
    paginatedItems.forEach((item, index) => {
        ...
        if (!isViewer) {
            if (isBorrowed) {
                ...
                checkboxHtml = `<td class="checkbox-column" onclick="event.stopPropagation();">
                    <input type="checkbox" class="item-checkbox" value="${item.id}" disabled style="opacity: 0.5; cursor: not-allowed; pointer-events: none;" title="You can only select items from your own department" />
                </td>`;
            } else {
                // Checkbox enabled - user can select this item
                checkboxHtml = `<td class="checkbox-column" onclick="event.stopPropagation();">
                    <input type="checkbox" class="item-checkbox" value="${item.id}" ${isSelected ? 'checked' : ''} onclick="event.stopPropagation();" onchange="toggleItemSelection(${item.id}, event)" style="cursor: pointer; pointer-events: auto; position: relative; z-index: 12;" />
                </td>`;
            }
        }
...
function showTableForItemTable(tableId, tableName) {
    ...
    // Check if user is a viewer/teacher - prevent them from viewing items
    const isViewer = document.body && document.body.dataset && document.body.dataset.userIsViewer === 'true';
    if (isViewer) {
        // Viewers/Teachers cannot view items in item tables
        const modalObj = window.modal || {};
        if (modalObj.warning) {
            modalObj.warning('You do not have permission to view items in this table. Please use the "Request to Borrow" option to request items.');
        } else {
            alert('You do not have permission to view items in this table. Please use the "Request to Borrow" option to request items.');
        }
        return;
    }
    
    // Security check: Department heads can only access tables from their own department
    const userIsDeptHead = (document.body?.dataset?.userIsDepartmentHead || 'false') === 'true';
    if (userIsDeptHead && typeof USER_DEPARTMENT_ID !== 'undefined' && USER_DEPARTMENT_ID !== null) {
        // First, fetch the table info to check its department
        fetch(`crud.php?action=get_item_tables`)
            .then(response => response.json())
            .then(data => {
                if (data.success && data.item_tables) {
                    const table = data.item_tables.find(t => String(t.id) === String(tableId));
                    if (table) {
                        const tableDeptId = String(table.department_id || '');
                        const userDeptId = String(USER_DEPARTMENT_ID);
                        
                        if (tableDeptId !== userDeptId) {
                            // Department head trying to access table from another department
                            const modalObj = window.modal || {};
                            if (modalObj.warning) {
                                modalObj.warning('You do not have permission to view items from this table. You can only access item tables from your own department.');
                            } else {
                                alert('You do not have permission to view items from this table. You can only access item tables from your own department.');
                            }
                            // Reset to card view
                            if (typeof updateCardView === 'function') {
                                updateCardView();
                ...
                // Security check passed, proceed to load items
                proceedToLoadTableItems(tableId, tableName);
            })
            .catch(error => {
                console.error('Error checking table access:', error);
                const modalObj = window.modal || {};
                if (modalObj.error) {
                    modalObj.error('Error verifying table access. Please try again.');
                } else {
                    alert('Error verifying table access. Please try again.');
                }
            });
    } else {
        // Not a department head, proceed normally
        proceedToLoadTableItems(tableId, tableName);
    }
}

function proceedToLoadTableItems(tableId, tableName) {
    ...
    fetch(`crud.php?action=get_items_by_table&table_id=${tableId}`)
        ...
        .then(data => {
            ...
            // Create single item table with selection functionality
            const sortedItems = sortItemsArray(tableItems);
            const currentPage = initializeCategoryPagination(tableName, sortedItems);
            const paginatedItems = getPaginatedItems(sortedItems, currentPage);
            
            const tableHTML = generateSelectableCategoryTableHTML(tableName, sortedItems, paginatedItems, currentPage);
            tableContainer.innerHTML = tableHTML;
            ...
            tableContainer.innerHTML = selectionContainer + tableContainer.innerHTML;
...
function updateSingleCategoryTable(categoryName, categoryItems) {
    try {
        ...
        console.error('Error in updateSingleCategoryTable:', error);
    }
}
...
function populateTableFromCards() {
    // Hide the static noFilterMessage immediately when this function is called
    const noFilterMessage = document.getElementById('noFilterMessage');
    if (noFilterMessage) {
        noFilterMessage.style.display = 'none';
    }
    ...
    } else if (!showAllItems && selectedDepartmentId !== 'all') {
        // For non-department heads: filter by selected department if showAllItems is false
    ...
    if (!itemsToShow || itemsToShow.length === 0) {
        // noFilterMessage is already hidden at the start of the function
        tableContainer.innerHTML = `
            <table class="data-table category-table">
                ...
                <tbody>
                    <tr><td colspan="8" class="no-items" style="text-align: center; padding: 20px;">No items found</td></tr>
                </tbody>
            </table>
        `;
        updateSortIndicators();
            ...
            if (isViewer) {
                ...
                actionHtml = `
                    <td onclick="event.stopPropagation();">
                        <div class="action-dropdown">
                            <button class="action-btn" onclick="event.stopPropagation(); toggleActionMenu(${item.id})">⋮</button>
                            <div class="action-menu" id="menu-${item.id}">
                                <button onclick="event.stopPropagation(); viewItem(${item.id})"><img src="image/view-details.png" alt="View" style="width:14px;height:14px;vertical-align:middle;margin-right:5px;" /> View Details</button>
                                ${(item.display_status || item.status || 'Unknown') !== 'Borrowed' ? `
                                ${(IS_SUPER_ADMIN || item.department_name === USER_DEPARTMENT) ? `
                                <button onclick="event.stopPropagation(); editItem(${item.id})"><img src="image/edit.png" alt="Edit" style="width:14px;height:14px;vertical-align:middle;margin-right:6px;" /> Edit</button>
                                <button onclick="event.stopPropagation(); archiveItem(${item.id})"><img src="image/icons8-archive-50.png" alt="Archive" style="width:14px;height:14px;vertical-align:middle;margin-right:6px;" /> Archive</button>
                                ` : ''}
                                ` : ''}
                            </div>
                        </div>
        ...
        <table class="data-table category-table">
                    <thead>
                        <tr>
                            ...
                            <th onclick="setSort('department_name')" class="sortable">Department <span class="sort-indicator" data-col="department_name"></span></th>
                            <th onclick="setSort('category')" class="sortable">Categories <span class="sort-indicator" data-col="category"></span></th>
                            <th onclick="setSort('quantity')" class="sortable">Quantity <span class="sort-indicator" data-col="quantity"></span></th>
                            <th onclick="setSort('updated_at')" class="sortable">Date Last Updated <span class="sort-indicator" data-col="updated_at"></span></th>
                            <th onclick="setSort('status')" class="sortable">Status <span class="sort-indicator" data-col="status"></span></th>
                            <th onclick="setSort('location')" class="sortable">Location <span class="sort-indicator" data-col="location"></span></th>
                            <th>Action</th>
    ...
    return fetch('crud.php')
        ...
        .then(data => {
            if (data.success) {
                ...
                // Items will only be displayed after user performs a search
                // BUT: If viewing a specific item table, don't show cards
                if (!window.currentTableId) {
                    updateSummary();
                } else {
                    // Just update summary without triggering card view
                    updateSummary();
                    // Ensure cards stay hidden when viewing a table
                    const cardsContainer = document.getElementById('itemsCardsContainer');
                    const tableContainer = document.querySelector('.table-container');
        ...
        .catch(error => {
            console.error('Fetch error while loading items:', error);
            
            // If we don't have any items loaded yet, show the error card
            if (!Array.isArray(allItems) || allItems.length === 0) {
                showNoItemsCard('Error connecting to server');
            } else {
                // If we already have items in memory, just keep showing them and refresh summary
                try {
                    updateSummary();
                    // Don't show cards if viewing a specific item table
                    if (!window.currentTableId && typeof updateCardView === 'function') {
                        updateCardView();
                    } else if (window.currentTableId) {
                        // Ensure cards stay hidden and table stays visible
                        const cardsContainer = document.getElementById('itemsCardsContainer');
                        const tableContainer = document.querySelector('.table-container');
                        if (cardsContainer && tableContainer) {
                            cardsContainer.style.display = 'none';
                    ...
                    console.error('Error refreshing UI after fetch failure:', e);
                }
            }
        })
        .finally(() => {
...
function viewItem(itemId, retryCount = 0) {
    console.log('Opening modal for item:', itemId);
    
    // Prevent infinite recursion
    if (retryCount > 1) {
        alert('Item not found. Please try refreshing the page.');
        return;
    }
    ...
    if (!item) {
        ...
        if (typeof loadAllItems === 'function' && (!allItems || allItems.length === 0)) {
            loadAllItems().then(() => {
                ...
                if (foundItem) {
                    // Retry opening modal with found item
                    setTimeout(() => {
                        viewItem(itemId, retryCount + 1);
                    }, 100);
                } else {
                    alert('Item not found. Please try refreshing the page.');
                }
...
function closeItemDetailModal() {
    ...
    // Ensure view state remains consistent (don't show item tables if we're in table view)
    const tableContainer = document.querySelector('.table-container');
    const cardsContainer = document.getElementById('itemsCardsContainer');
    if (tableContainer && typeof currentTableItems !== 'undefined' && currentTableItems.length > 0) {
        if (cardsContainer) {
            cardsContainer.style.display = 'none';
        }
        tableContainer.style.display = 'block';
